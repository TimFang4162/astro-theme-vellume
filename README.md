...
- **Skins** — reskin the whole site by changing one config key: the built-in `default` / `material` looks, with the code-highlighting, OG-image, and browser-chrome colors following along. An optional print template (`theme.print`, `thesis`) makes printing always render as paper regardless of the screen skin. Dark values are screen-gated at build time, so printing always renders light paper (see `docs/theming.md`).
