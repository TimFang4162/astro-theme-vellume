...
- **Skins** — reskin the whole site by changing one config key: the built-in `default` / `material` / `thesis` looks, with the code-highlighting, OG-image, and browser-chrome colors following along. Every skin ships to every visitor and is switchable at runtime from the header palette; dark values are screen-gated at build time, so printing always renders light paper (see `docs/theming.md`).
