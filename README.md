...
- **Skins** — reskin the whole site by changing one config key: the built-in `default` / `material` looks, with the code-highlighting, OG-image, and browser-chrome colors following along. Printing defaults to the `thesis` paper template (`theme.print`), so paper always renders as thesis regardless of the screen skin (set `print: ""` to let print follow the screen skin's light half). Dark values are screen-gated at build time, so printing always renders light paper (see `docs/theming.md`).
