...
Print styling is a single fixed floor in `src/styles/print.css` (plain `@media print`): hide chrome, flatten to one column, wrap code instead of scrolling, expand non-anchor content links as footnotes (excluding `mailto:`/`tel:`), force white paper, pagination hygiene. The `theme.print` layer (default `thesis`) is injected before it, so the template's paper typography (serif/indents/centred titles/chapter breaks) overrides the floor's flattening; with `print: ""` disabled, the profile skin's light half carries into paper. See `docs/theming.md`.
