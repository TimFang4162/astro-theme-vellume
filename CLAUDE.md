...
Tailwind CSS v4 with `@import "tailwindcss"` syntax. Main stylesheet: `src/styles/global.css`, which imports the concern partials in cascade order (tokens → base → components → prose → code-block → transitions → utilities → toc → print; toc/transitions/print are deliberately unlayered so they outrank layered rules). The active theme profile's tokens are injected between global.css and the user files via the `vellume-theme-profile` virtual module (Vite plugin in astro.config.ts), so bundle order enforces the precedence chain tokens.css < profile < theme.css < custom.css. Dark mode uses `[data-theme="dark"]` attribute (not `prefers-color-scheme`), toggled via `astro-theme-toggle`; `color-scheme` is declared per theme in tokens.css and mirrored inline by the swap handler in BaseLayout. Design tokens are CSS custom properties mapped to Tailwind's `--color-*`/`--font-*` namespaces via `@theme inline`. User overrides go in `src/site/theme.css`.
...
### Theme Profiles & Print

One theme = one css file under `src/site/profiles/` (screen profiles: light `:root` block + optional `[data-theme="dark"]` block; print profiles under `profiles/print/`: one flat `:root` token block), plus a small typed registration in `src/config/theme-profiles.ts` (`themeProfiles` / `printProfiles`: file, menu label, `states`, and `meta` branding for the non-CSS color consumers — browserColor, shiki themes, OG palette, mermaid themeVariables — resolved by `resolveThemeBranding()`; explicit user config outranks profile meta, which outranks `theme-default.ts` defaults). `theme.profile` / `theme.print` select registered names; unknown names warn and fall back to `default`. The system is semantic-free: it injects the active profile's css (verbatim for screen; print tokens re-scoped to `[data-print="<name>"]` for the visitor menu) and relays state attributes — all structural variants are theme-owned CSS keyed on those attributes. See `docs/theming.md`.

Print styling is attribute-driven: rules live under `[data-print-style]` on `<html>`, mirrored from `beforeprint`/`afterprint` by an inline head script (so Ctrl+P works) and held for the whole preview session. Print profiles ship tokens scoped to `[data-print-style][data-print="<name>"]`; BaseLayout stamps `data-print` from the owner's `theme.print`. The article title's three-dot menu (`ArticleActions.astro` + `scripts/ui/article-actions.ts`) offers copy link / share / print, where print enters a preview session: `PrintPreviewPanel.astro` + `scripts/ui/print-preview.ts` show a config panel (profile, links, images, chapter breaks) that toggles the root attributes live — the panel is screen-only (`@media print` hides it). Structural variants are theme-owned CSS keyed on those attributes; `@media print` itself only carries the no-JS essentials fallback, the panel hide, and `@page`.
