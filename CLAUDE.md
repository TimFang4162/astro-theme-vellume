...
Print profiles are activated by the `data-print` root attribute, stamped by BaseLayout from the owner's `theme.print` (visitor-facing per-print-profile switching is not exposed; the article title's three-dot menu in `ArticleActions.astro` + `scripts/ui/article-actions.ts` offers copy link / share / print, where print uses the active profile). print.css implements structural variants keyed on `data-print` plus the relayed state vocabulary `data-print-links`/`data-print-images`/`data-print-break`.
